...
class NotificationService(
        ...
        if not self._report_summary_only:
            for result in sorted_results:
                ...
                if battle:
                    ...
                    # 具体买卖指导（新增）
                    exec_guide = battle.get('execution_guide', {}) if battle else {}
                    if exec_guide and exec_guide.get('order_instruction'):
                        report_lines.extend([
                            f"**📋 明日操作指令**",
                            "",
                            f"> {exec_guide['order_instruction']}",
                            "",
                            f"| 操作方式 | 说明 |",
                            f"|---------|------|",
                            f"| 订单类型 | {exec_guide.get('order_type', '-')} |",
                        ])
                        if exec_guide.get('limit_price_detail'):
                            report_lines.append(f"| 挂单价格 | {exec_guide['limit_price_detail']} |")
                        if exec_guide.get('suggested_lots'):
                            report_lines.append(f"| 建议数量 | {exec_guide['suggested_lots']} |")
                        report_lines.append("")
                    # 检查清单
    ...
    def generate_wechat_dashboard(self, results: List[AnalysisResult]) -> str:
        ...
        if self._report_summary_only:
            ...
            for result in sorted_results:
                ...
                exec_guide = battle.get('execution_guide', {}) if battle else {}
                if exec_guide and exec_guide.get('order_instruction'):
                    lines.append(f"📋 **明日操作**: {exec_guide['order_instruction']}")
                    parts = []
                    if exec_guide.get('order_type'):
                        parts.append(exec_guide['order_type'])
                    if exec_guide.get('suggested_lots'):
                        parts.append(exec_guide['suggested_lots'])
                    if parts:
    ...
    def generate_single_stock_report(self, result: AnalysisResult) -> str:
        ...
        exec_guide = battle.get('execution_guide', {}) if battle else {}
        if exec_guide:
            order_instruction = exec_guide.get('order_instruction', '')
            if order_instruction:
                ...
                limit_price = exec_guide.get('limit_price_detail', '')
                if limit_price:
                    lines.append(f"| 挂单价格 | {limit_price} |")
                suggested_lots = exec_guide.get('suggested_lots', '')
                if suggested_lots:
                    lines.append(f"| 建议数量 | {suggested_lots} |")
                lines.append("")
